...
def _fit_failure_predictor(
    ...
    if len(x_hist) < 4:
        return float(np.mean(y_hist)) if y_hist else 0.5
    ...
    model.fit(X, y)
    return float(np.clip(model.predict_proba(X[-1:].copy())[:, 1][0], 0.0, 1.0))
    ...
    *,
    ...
    for t, fold in enumerate(ordered):
        ...
        for name in models:
            ...
            risk_value = _fit_failure_predictor(next_ready, next_labels) if next_ready else 0.5
            if next_labels:
