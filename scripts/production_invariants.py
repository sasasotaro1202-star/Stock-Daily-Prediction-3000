...
def main():
    ...
    checks=[
        ...
        ("on_demand_production_prediction",Path(".github/workflows/on-demand-production-prediction.yml").exists() and "workflow_dispatch:" in Path(".github/workflows/on-demand-production-prediction.yml").read_text() and "load_production_artifact" in Path(".github/workflows/on-demand-production-prediction.yml").read_text()),
        ...
        ("actions_reliability_watchdog",Path(".github/workflows/actions-reliability-watchdog.yml").exists() and 'cron: "7 * * * *"' in Path(".github/workflows/actions-reliability-watchdog.yml").read_text() and "bounded recovery" in Path(".github/workflows/actions-reliability-watchdog.yml").read_text()),        ("cancelled_run_recovery","conclusion == 'failure'" in Path(".github/workflows/bounded-production-recovery.yml").read_text() and "conclusion == 'cancelled'" in Path(".github/workflows/bounded-production-recovery.yml").read_text() and "run_attempt == 1" in Path(".github/workflows/bounded-production-recovery.yml").read_text()),        ("automation_heartbeat",Path(".github/workflows/heartbeat.yml").exists() and 'cron: "17 */6 * * *"' in Path(".github/workflows/heartbeat.yml").read_text()),        ("live_performance_gate","live_performance_gate.py" in Path(".github/workflows/us-close-prediction.yml").read_text() and Path("scripts/live_performance_gate.py").exists()),
