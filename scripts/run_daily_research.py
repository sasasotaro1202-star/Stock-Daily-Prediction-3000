...
def make_models():
    return {
        ...
        "extra_trees": lambda: make_pipeline(SimpleImputer(strategy="median"), ExtraTreesClassifier(
            n_estimators=300,min_samples_leaf=20,n_jobs=-1,random_state=42)),
        "hgb": lambda: make_pipeline(SimpleImputer(strategy="median"), HistGradientBoostingClassifier(
...
def main():
    ...
    audit={"feature_columns":list(FEATURE_COLUMNS),
           "ok":feature_audit.ok and separation.ok,
    ...
    for name,factory in make_models().items():
        ...
        for fold in folds:
            ...
            if len(core)<1000 or len(cal)<50 or len(test)<50:
                continue
            if core.target_up_1d.nunique()<2 or cal.target_up_1d.nunique()<2 or test.target_up_1d.nunique()<2:
                continue
            model=factory()
    ...
    payload={"results":results,"selected_model":selected,
             "selection_basis":"minimum mean chronological OOS logloss; calibration fit only inside each training fold; frozen holdout unused",
             "status":"OOS_COMPLETE"}
