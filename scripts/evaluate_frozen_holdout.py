...
def main():
    ...
    # Use the exact production ranking implementation. The uncertainty input
    # is routed through the same asset-specific q10/q90 models used in
    ...
    ranking_frame = pd.DataFrame(
        {
            ...
            "ranking_uncertainty": np.zeros(len(test), dtype=float),
        },
        index=test.index,
    )
    ...
    for asset, subset in test.groupby("asset_class", sort=False):
        qmodels = q_assets.get(str(asset), q_global)
        pos = ranking_frame.index.get_indexer(subset.index)
        lo_vals = qmodels["q10"].predict(subset[FEATURE_COLUMNS])
        hi_vals = qmodels["q90"].predict(subset[FEATURE_COLUMNS])
