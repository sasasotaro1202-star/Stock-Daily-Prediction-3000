...
PRICE=Path("data/prices"); METRICS=Path("data/research/latest_metrics.json"); OUT=Path("data/predictions/latest.parquet")

def models():
    return {
        "logistic":make_pipeline(SimpleImputer(strategy="median"),LogisticRegression(max_iter=1000,C=0.5)),
        "extra_trees":make_pipeline(SimpleImputer(strategy="median"),ExtraTreesClassifier(n_estimators=300,min_samples_leaf=20,n_jobs=-1,random_state=42)),
        "hgb":make_pipeline(SimpleImputer(strategy="median"),HistGradientBoostingClassifier(max_iter=300,learning_rate=0.04,max_leaf_nodes=31,l2_regularization=1.0,random_state=42)),
    }

def main():
    if not PRICE.exists() or not METRICS.exists(): raise SystemExit("DEFERRED: research artifacts are missing")
    payload=json.loads(METRICS.read_text(encoding="utf-8")); selected=payload.get("selected_model")
    if selected not in models(): raise SystemExit("DEFERRED: selected model unavailable")
    ...
    prediction_date=max(pd.to_datetime(df["session_date"]).dt.date)
    latest=df[df["session_date"].eq(prediction_date)].copy()
    ...
    latest["p_up_1d"]=p; latest["expected_return_1d"]=expected
    ...
    latest["prediction_date"]=prediction_date; latest["model_id"]=f"{selected}-oos-v1"
    out=cross_sectional_rank(latest[["symbol","asset_class","session_date","close","prediction_time","prediction_date","p_up_1d","expected_return_1d","expected_close_1d","range_low_1d","range_high_1d","model_id"]])
